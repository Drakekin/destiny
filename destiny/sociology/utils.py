...
def generate_earth_pops(rng: Random, population_multiplier: float = 10.0 / 8, earth: Planet = None) -> InhabitedPlanet:
    ...
    for country in earth_pop_json:
        ...
        for _ in range(int(adjusted_population // POP_TARGET_SIZE)):
            population = Population(
                rng, POP_TARGET_SIZE, [(country["country"], 100)], randomise_statistics=True
            )
            population.children = [rng.randint(int(50/10_000*POP_TARGET_SIZE), int(300/10_000*POP_TARGET_SIZE)) for _ in range(20)]
            pops.append(population)
        if len(pops) == 0:
            continue
        settlement = Settlement(rng, country["country"], pops, rng.choice(GOVERNMENTS))
