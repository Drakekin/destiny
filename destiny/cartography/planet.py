...
class Planet:
    ...
    def __init__(
        ...
        orbital_radius: float,
        solid: bool,
        surface_water: Optional[float] = None,
    ...
    def generate_life(self, rng: Random):
        ...
        if self.surface_temperature > 100:
            ...
            options = (
                [None] * 35
                + [LifeType.exotic] * 3
                + [LifeType.human_neutral, LifeType.human_hostile]
            )
        elif not self.solid:
            options = (
                [None] * 10
                + [LifeType.exotic] * 5
                + [LifeType.human_neutral] * 2
                + [LifeType.human_hostile]
            )
        elif not self.habitable:
            options = (
                [None] * 5
                + [LifeType.human_hostile] * 7
                + [LifeType.human_neutral] * 2
                + [LifeType.human_compatible, LifeType.exotic]
            )
        elif self.habitable:
            options = (
                [None] * 5
                + [LifeType.human_hostile, LifeType.human_neutral] * 5
                + [LifeType.human_compatible] * 4
                + [LifeType.exotic]
            )
    ...
    def orbital_period(self):
        ...
        period_seconds = (
            2
            * math.pi
            * math.sqrt((orbit_metres**3) / (gravitational_constant * star_mass_kg))
        )
    ...
    def __repr__(self):
        ...
        if self.solid:
            ...
            life_str = (
                f"{self.native_life.value} {LIFE_LEVEL_NAMES[self.life_level.value]}, "
                if self.native_life
                else "probably "
            )
            habitable_str = (
                f"{life_str}habitable" if self.habitable else f"{life_str}uninhabitable"
            )
